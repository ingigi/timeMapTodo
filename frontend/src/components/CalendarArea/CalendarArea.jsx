        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-md">
            <button 
              ...
              className={`px-3 py-1 text-sm font-semibold rounded ${viewType === 'week' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
            >
              週
            </button>
            <button 
              onClick={() => setViewType('month')}
              className={`px-3 py-1 text-sm font-semibold rounded ${viewType === 'month' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
            >
              月
            </button>
          </div>
          ...
          <div className="flex items-center rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">
            <button onClick={handlePrev} className="p-1 hover:text-blue-600"><ChevronLeft className="h-4 w-4" /></button>
            <span className="cursor-pointer mx-2 hover:text-blue-600" onClick={handleToday}>
              {formattedDateRange()}
            </span>
            <button onClick={handleNext} className="p-1 hover:text-blue-600"><ChevronRight className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden relative">
        {viewType === 'week' ? (
          <WeeklyBoard
            ...
            onUpdateCapacities={onUpdateCapacities}
